...
var async = require('async'),
	fs = require('fs'),
	configFile = __dirname + '/../conf/settings.js';

if (!fs.existsSync(configFile)) {
	console.error('"conf/settings.js" file does not exist!\n');
	console.error('Copy "conf/settings.example.js" to "conf/settings.js", then change as desired.\n');
	process.exit(1);
}
	...
	(function run() {
		async.whilst(
			...
			function (err) {
				...
				setTimeout(run, config.refreshInterval || 2000);
			}
		);
	})();
});
